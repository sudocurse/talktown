...
class Names(object):
    ...
    miscellaneous_masculine_forenames = tuple(
        name[:-1] for name in
        open(os.getcwd()+'/corpora/DnD-characters/fnames.txt', 'r')
    )
    miscellaneous_feminine_forenames = tuple(
        name[:-1] for name in
        open(os.getcwd()+'/corpora/DnD-characters/fnames.txt', 'r')
    )
    fantasy_surnames = tuple(
        name.strip('\n') for name in
        open(os.getcwd()+'/corpora/DnD-characters/lnames.txt', 'r')
    )
    ...
    all_surnames = (
        fantasy_surnames + scandinavian_surnames
    )
    place_names = tuple(
        name.strip('\n') for name in
        # open(os.getcwd()+'/corpora/US_settlement_names.txt', 'r')
        open(os.getcwd()+'/corpora/DnD-characters/long_names.txt', 'r')
    )
    ...
    def a_feminine_name(cls, year):
        ...
        if x > 0.99:
            # Choose any masculine name (allows rare ones to be used occasionally)
            name = random.choice(cls.miscellaneous_feminine_forenames)
        else:
            # Choose using the actual distribution of American names this decade
